...
| `rust-unsafe-safety-contract-verification` | Use when auditing unsafe Rust: every unsafe block must carry a SAFETY comment whose preconditions are real and encoded in the type system. Prevents fabricated contracts — like Bun PathString.rs (2026) — that claim a 'caller guarantees' invariant no type enforces, silently permitting use-after-free. | source-backed | `skills/rust/rust-unsafe-safety-contract-verification` |
