...
1. **Web-ресёрч failure-кейсов**: подтверждены первичные источники для новых скиллов —
   MISRA C++ LLM violations 23-29/KLOC и 0 полностью compliant моделей (Research Square
   rs-8123173, arXiv 2506.23535), Top-k → −83%; agent deception (PNAS 2317967121,
   arXiv 2509.03518 «Can LLMs Lie?», arXiv 2509.18970 survey); differential compiler
   testing (UBfuzz ASPLOS'24, DiffSpec 2410.04249, DESIL 2504.01379); Checked C
   (3C, arXiv 2203.13445). Gap-анализ: IEEE 754 и endianness не покрыты → +2 скилла.

2. **22 новых скилла** (5 батчей параллельных субагентов), 8 source-backed с реальными
   host-прогонами (MISRA Top-k checker 11 findings/clean, evidence-gate 2/4 FAIL,
   refactor diff-guard −3057 LOC FLAG, -O0/-O2 signed-overflow divergence,
   canary 0xC0000409 vs silent, gdb post-mortem stack corruption, fast-math errno=0
   divergence, byte-order golden vector PASS) и 14 researched с host-верифицированными
   python-моделями (MTE sim 6/6, io_uring ring protocol 8 bug-классов, CO-RE relocation,
   ...
   NAPI state machine 3/3, angr 9.2.213 установлен и нашёл magic input).

3. **НОВЫЙ домен safety/** (misra-c-compliance, hard-real-time-determinism); расширены
   _meta, compiler, c, kernel, networking, security, embedded, gpu, debugging.

4. **Реестр интегрирован** (one-shot integrate_new_skills.py, затем удалён):
   skills.yaml 185 (93 source-backed/92 researched), sources.yaml 278 (+50), claims.yaml
   184 (CL-142..CL-184), cross-links.yaml 322 (+89). registry_check 0 циклов.

5. **Документация полностью обновлена**: README (185/35/93/278/184, v3.0-секция,
   area-map с safety), docs/SKILLS.md + 35 domain README + docs/assets/skills.js
   перегенерированы, index.md, llms.txt, roadmap.md, agents-failures-cheatsheet.md
   (+секции 6-8), CHANGELOG (v3.0 Unreleased), architecture.md, roadmap/progress.yaml.

6. **Token gate**: core-dump-analysis (2101→1983) и rust-for-linux-module-dev
   (2030→1999) ужаты; validate.py ALL CHECKS PASSED (skill_lint 185/185).
...
1. **���������**: 163 skills / 34 ������ / 85 source-backed / 78 researched; registry 228 sources, 141 claims, 233 cross-links (skills.yaml summary ������������ 163/85/78).
2. **tools/validate.py v2.0** � �����������: skill_lint > token gate (--check 2000) > registry_check > source_check > claim_extractor > prose_lint (advisory, sampling 3); exit 0/1.
3. **������� CI (ci.yml)**: ������������ ����� `validate` �� ubuntu-latest; bash-`find` ��� token gate (�� cross-platform!); freshness-����: gen_skills_catalog>docs/SKILLS.md (fail), gen_landing_data>docs/assets/skills.js (fail), gen_domain_readmes>warning-only. `permissions: contents: read`.
4. **Windows-�������������**: `find`/bash-����������� �� �������� � pwsh; requirements-dev.txt �������� ������ PyYAML>=6.0; semgrep/clang �� ����������� �� dev-�����, �� ���� � ubuntu-latest.
5. **�����������**: docker/ (�������), Makefile (�������), tools/eval_runner.py (������ ���� B � � CI ��������� �� merged tree), registry/skills.min.yaml, registry/triggers.yaml, tools/tool_schema.yaml, tools/sanitizer_run.sh (������ ����).
6. **token_measure.py** CLI: `--check 2000 <skill-dir...>`; tiktoken ���������� (fallback-heuristic). ���s-��������� ����� python-���� � cross-platform.
7. **stale.yml** ���������� � �� �������. Git: ����� main, ������ ������ (head 73a400e).

## 2026-08-19 � Session PHASE-C (v3.0)

1. **C1: CI/CD matrix** � `.github/workflows/ci.yml` ���������: `validate` (matrix ubuntu/windows, skill_lint+registry_check+source_check ����� validate.py + ����� cross-platform token gate python -c-������ + 3 freshness-����), `eval-source-backed` (eval_runner.py --source-backed-only), `eval-security` (semgrep p/cwe-top-25 � --error + CodeQL init/autobuild/analyze, continue-on-error), `eval-researched-stub` (eval_runner.py ������ ������), `token-budget` (������ ���� 2000). `permissions: contents: read` ���������. YAML �������� (python yaml.safe_load OK).
2. **C2: docker/Dockerfile.ubuntu-eval** � ubuntu:24.04: build-essential/gcc/g++/clang/lld/binutils/nasm/cmake/ninja/qemu-user+qemu-system-x86/python3/pip/venv/git/curl; rustup stable+nightly (� miri); venv � semgrep; CodeQL CLI (releases/latest/download); RUN-����������� ������ ���� ���������.
3. **C3: Makefile** � ������� validate/eval/security/test (+����� token), TAB-������� ���������, 4/4 ������� �� �����.
4. **�������**: `feat(ci): add multi-OS matrix with security and eval jobs`, `feat(docker): add ubuntu evaluation environment`, `feat(build): add Makefile with validate, eval, security targets`.
5. **Discrepancy**: �� ����� ��� tools/eval_runner.py (������ ���� B) � CI-������ �� ���� ���������� �� merged tree; registry/skills.min.yaml + triggers.yaml + gen_skills_min.py ��������� � ������ �� ������������ ���� � �� ���������.
## Pre-flight analysis by agent
...
3. Frontmatter is uniform (name + description only, <=50 words); body max 171 lines (kernel-ub-patterns); token worst ~1689 (heuristic) — adding a ~50-token CRITICAL REMINDER stays under the 2000 gate and 250-line cap.
4. Missing (my scope): tools/restructure_skills.py, skills/_meta/meta-compaction/, skills/_meta/meta-self-consistency/, registry entries + summary update (163→166, tier_0 6→9, researched 78→81), regeneration of docs/SKILLS.md, docs/assets/skills.js, .claude-plugin/marketplace.json, skills/_meta/README.md.
5. meta-silent-failure-detection (Phase E) does NOT exist yet — must be registered with status: registered (registry_check WARNs only on 'implemented' with missing SKILL.md), lead promotes in Phase 7.
6. Reserved: skills/_meta/meta-routing/ and skills/_meta/agent-tool-whitelist/ (phases A/E) — restructure script must exclude them.
7. Generators confirmed: gen_skills_catalog.py, gen_landing_data.py, gen_marketplace.py, gen_domain_readmes.py — all read registry/skills.yaml; CI freshness-gates docs/SKILLS.md and docs/assets/skills.js. Pre-existing WORKLOG section was rewritten from broken encoding to UTF-8 (discrepancy note).
