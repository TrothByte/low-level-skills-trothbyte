...
[![skills](https://img.shields.io/badge/skills-193-0F766E?style=flat-square)](docs/SKILLS.md)
[![domains](https://img.shields.io/badge/domains-35-475569?style=flat-square)](#skills-catalog)
[![source-backed](https://img.shields.io/badge/source--backed-101-15803D?style=flat-square)](registry/skills.yaml)
[![primary sources](https://img.shields.io/badge/primary--sources-290-1D4ED8?style=flat-square)](registry/sources.yaml)
[![traced claims](https://img.shields.io/badge/traced--claims-248-9D174D?style=flat-square)](registry/claims.yaml)
...
> [!NOTE]
> **193 skills · 35 domains · 101 source-backed · 290 primary sources · 248 traced claims.**
...
## v3.1: Phase 1 of the expansion pipeline — kernel & networking core

The first slice of the [expansion pipeline](trothbyte_skills_expansion_pipeline.md):
8 kernel/networking core skills that cover the most common sources of kernel panics
and data loss. All `source-backed` with host-verified compile-and-run examples:

- **Kernel**: [`vfs-file-operations-and-fops`](skills/kernel/vfs-file-operations-and-fops/SKILL.md),
  [`page-cache-writeback-semantics`](skills/kernel/page-cache-writeback-semantics/SKILL.md),
  [`workqueue-flush-and-cancellation`](skills/kernel/workqueue-flush-and-cancellation/SKILL.md),
  [`kernel-timers-hrtimer-vs-legacy`](skills/kernel/kernel-timers-hrtimer-vs-legacy/SKILL.md),
  [`waitqueue-completion-synchronization`](skills/kernel/waitqueue-completion-synchronization/SKILL.md),
  [`kthread-create-and-teardown`](skills/kernel/kthread-create-and-teardown/SKILL.md)
- **Networking**: [`sk-buff-socket-buffer-management`](skills/networking/sk-buff-socket-buffer-management/SKILL.md),
  [`tcp-congestion-control-internals`](skills/networking/tcp-congestion-control-internals/SKILL.md)

Phase 1 also added 12 primary sources (Linux kernel VFS/writeback/workqueue/timer/
completion/kthread/sk_buff/networking/TCP docs + RFC 793/5681/8312) and 64 traced
claims (CL-185..CL-248). Next: Phase 2 (concurrency + security mitigations).

## Repository map

| Path | Purpose |
|---|---|
| `skills/` | 193 skills across 35 domains (`SKILL.md` + `references/` + `examples/` + `evals/`) |
...
- **Every normative claim is traceable:** `claim → source → section → skill` in
  [`registry/claims.yaml`](registry/claims.yaml), backed by 290 primary sources in
