// intentionally incorrect — FAKE parallelism. Four threads are created and a
// global mutex serializes the entire workload: max_started=4 (all threads
// alive and queued) but max_working=1 (only one executes at a time) and the
// wall time is ~4x the real-parallel version. Thread-safe primitives alone do
// not make code parallel — this is the CONCUR "ST" failure class.
#include <pthread.h>
#include <stdint.h>
#include <stdio.h>
#include <time.h>
...
int main(void)
{
    ...
    printf("fake_parallel: threads=%d cores=%lu wall=%.3fs "
